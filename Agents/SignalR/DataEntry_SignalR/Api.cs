...
internal sealed class Api
{
    ...
    public Task<PromptResponse?> PromptToFillAsync(
        ...
        Dictionary<string, string> data)
    {
        ...
        return _kernel.InvokePromptAsync<PromptResponse>($@"Here are the set of fields the user needs to fill out. If a description begins with `RESPONSE`, then it does *not* need to be filled out, but rather the description IS the user's response that they've already furnished after being prompted for it.
...
Remember, you must only ask for one field at a time.", _kernelArgs);
    }
}
